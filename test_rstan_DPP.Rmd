...
#' @returns ggplot grob
#' 
#' @param stanfit stanfit object containing estimated cluster parameters
...
plot_cluster_parameters <- 
  function(stanfit
    ...
    # summarize estimates of location & size
    loc_estimates <- get_sampled_parameter(stanfit
                               , pars = stanfit_loc_var
                               , parameter_value = d_cluster_val
                               , parameter_id = d_cluster_id
                               )
    size_estimates <- get_sampled_parameter(stanfit
                            , pars = stanfit_size_var
                            , parameter_value = 'prop'
                            , parameter_id = d_cluster_id
                            )

    estimates <- loc_estimates %>%
      inner_join(size_estimates
                 , by = c(d_cluster_id, 'iter')
      ) %>%
      mutate(source = 'estimated clusters')
    
    # plot estimated cluster parameters
    p <- ggplot() + 
      geom_density_2d(data = estimates
    ...
    true_values <- d %>%
      ...
      summarize_each(funs = funs(mean), matches(d_cluster_val), prop) %>%
      ungroup() %>%
